...
<nav id="menu-1" class="mega-menu <?php echo esc_attr($dashboard_class); ?>">
    	...
    	<?php if (is_user_logged_in()) { ?>
            <section class="menu-list-items testclass">
        <?php } else { ?>
            <section class="menu-list-items no-login-user-menu">
        <?php } ?>
        ...
        <ul class="menu-button left-profile-main-menu">
            <?php
            if (is_user_logged_in()) {
                /* User Icon And Button */
                if ((isset($nokri['user_bar_switch'])) && $nokri['user_bar_switch'] == '1') {
                    ?>
                    <li class="profile-pic dropdown left-profile-main-menu">
                            ...
                            <?php 
                                if(is_user_logged_in())
                                {
                                    ...
                                    if ( metadata_exists( 'user', $user_id, $meta_key ) ) 
                                    {
                                        ...
                                        $last_message_time = $wpdb->get_results( "SELECT datetime FROM ej_user_messages WHERE id_user = " . $user_id . " ORDER BY datetime DESC LIMIT 1" );
                                        $last_message_time = $last_message_time[0]->datetime;
                                        $user_last_seen  = strtotime($user_last_seen);
                                        $last_message_time =   strtotime($last_message_time);
										$message_count = $wpdb->get_results( "SELECT COUNT(datetime) FROM ej_user_messages WHERE datetime > " . $user_last_seen );
                                        ...
                                        {  ?>
                                            <li><a href="https://job.co.zm/candidates/" class="message-link"><p><span class="message-counter"><?php echo($message_count);?></span><i class="fa fa-envelope" aria-hidden="true"></i></p></a></li>
